...
public class Main {
    ...
    private static void startGame(final Map<String, String> positions) {
        ...
        for (int i = 0; i < BOARD_LIMIT; i++) {
            ...
            for (int j = 0; j < BOARD_LIMIT; j++) {
                // Pega a configuração ou usa valor padrão "0,false"
                var positionKey = "%s,%s".formatted(i, j);
                var positionConfig = positions.getOrDefault(positionKey, "0,false");

                // Divide a string para extrair os valores
    ...
    public static void finishGame() {
        ...
        if (board.gameIsFinished()) {
            ...
            System.out.println("Você ainda não preencheu todos os espaços");
        }
